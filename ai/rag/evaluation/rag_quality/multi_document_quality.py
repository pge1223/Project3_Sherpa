...
def evaluate_multi_document_cases(
    ...
    *,
    ...
    verified = [case for case in cases if case.get("human_verified") is True]
    ...
    for case in verified:
        ...
        if gold:
            ...
            planner_cases += 1
            covered += int(bool(gold & selected))
        selected_total += len(selected)
        selected_relevant += len(selected & gold)
    ...
    return {
        ...
        "excluded_unverified_case_count": len(cases) - len(verified),
...
if __name__ == "__main__":
    ...
    print(json.dumps(evaluate_file(args.input, args.output), ensure_ascii=False, indent=2))
