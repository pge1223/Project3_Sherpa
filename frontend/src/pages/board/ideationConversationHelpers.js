...
const PHASE_LABEL_KO = {
  ...
  awaiting_user_decision: '사용자 확정 대기',
  finalized: '완료',
  failed: '실패',
}
...
export function statusLabelFor({ phase, starting, sending, finalizing }) {
  ...
  if (sending) {
    if (phase === 'awaiting_developer_answer' || phase === 'awaiting_user_decision') return '전문가 의견 생성 중'
    return '응답을 준비하는 중'
  }
...
// 사용자가 무엇을 더 해야 하는지 안내"). awaiting_user_decision은 확정 버튼 자체가
// 활성화되므로 별도 안내가 필요 없어 빈 문자열을 반환한다.
