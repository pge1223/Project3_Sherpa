...
async def ask_committee(
    ...
    """STEP7 "대화형 피드백" — 저장된 회의 결과를 근거로 위원장이 후속 질문에 답한다.
    개별 위원 재평가(reevaluate_reviewer)와 달리 재채점·재저장 없는 1회성 Q&A라 훨씬
    가볍다. 대화 기록은 서버에 저장하지 않고 매 요청마다 프론트가 history로 넘긴다."""
    get_current_user(authorization)
    ...
    llm_call = _build_real_llm_call(stored["meeting_id"])
    raw = await run_in_threadpool(llm_call, prompt)
    try:
        parsed = json.loads(raw)
    ...
    chair_card = get_persona_card("review_chair")
    return AskQuestionResponse(persona_id="review_chair", display_name=chair_card["display_name"], answer=answer)
